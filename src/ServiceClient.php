...
class ServiceClient implements ServiceClientInterface
{
     ...
     * @param callable $requestTransformer A callable that transforms a Command
     *     into a Request. The function should accept a
     *     `GuzzleHttp\Command\CommandInterface` object and return a
     *     `Psr\Http\Message\RequestInterface` object.
     * @param callable $responseTransformer A callable that transforms a
    ...
    public function __construct(
        ...
        callable $requestTransformer,
        callable $responseTransformer,
    ...
    public function batch($commands, array $options = [])
    {
        ...
        // Execute the batch synchronously, then sort and return the results.
        return $this->batchAsync($commands, $options)
    ...
    public function batchAsync($commands, array $options = [])
    {
        ...
        $promises = function () use ($commands) {
            foreach ($commands as $k => $command) {
                ...
                yield $k => $this->executeAsync($command);
            }
        };
        ...
        return function (CommandInterface $command) {
            return Promise\coroutine(function () use ($command) {
                ...
                try {
                    ...
                    $request = $this->transformers['request']($command);
                    $promise = $this->httpClient->sendAsync($request, $opts);

                    // Create a result from the response.
                    /** @var ResponseInterface $response */
                    $response = (yield $promise);
                    yield $this->transformers['response']($response, $request);
                } catch (\Exception $e) {
